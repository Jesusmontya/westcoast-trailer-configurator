...
function TurntableSurface({ rotation, setInteracting }: { rotation: React.MutableRefObject<number>; setInteracting: (value: boolean) => void }) {
  ...
  useFrame(() => {
    ...
    if (rotation.current >= limit) { rotation.current = limit; direction.current = -1; }
    if (rotation.current <= -limit) { rotation.current = -limit; direction.current = 1; }
  });
  ...
  return <mesh position={[0, 0, 0]} scale={[11, 7, 1]} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up}>
    <planeGeometry args={[1, 1]} /><meshBasicMaterial transparent opacity={0} depthWrite={false} />
  </mesh>;
}
      ...
      <div className="mx-auto mt-12 max-w-6xl">
        <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#17191c] shadow-2xl">
          ...
          <div className="relative h-[390px] w-full overflow-hidden bg-[#111315] sm:h-[500px] lg:h-[610px] touch-pan-y">
            {!loaded && <div className="absolute inset-0 z-25 flex items-center justify-center bg-[#111315]"><img src="/photos/hero-trailer.jpg" alt="Custom trailer preview" className="absolute inset-0 h-full w-full object-cover opacity-35" /><div className="relative z-10 rounded-full border border-white/10 bg-black/65 px-5 py-3 font-mono text-[9px] uppercase tracking-[0.2em] text-white/75 backdrop-blur-md">Interactive 3D preview loading…</div></div>}
            <Canvas camera={{ position: [5.5, 2.5, 6.5], fov: 30 }} dpr={[1, 1.5]} gl={{ antialias: true, alpha: true }}>
              <ambientLight intensity={2} /><directionalLight position={[5, 8, 5]} intensity={3} /><Environment preset="city" />
              <Suspense fallback={null}><TrailerModel rotation={rotation} onLoaded={() => setLoaded(true)} /></Suspense>
